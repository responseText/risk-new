...
function ajaxEdit(id)
{
  ...
  if(confirm( txt['edit-confirm']  ))
  {
    ...
    $('#myForm').attr("action", 'violencelevel/'+id+'/edit');
    $('#myForm').submit();
...
function ajaxShow(id)
{
  ...
  if(confirm( txt['show-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('get');
    $('#myForm').attr("action", 'violencelevel/'+id);
    $('#myForm').submit();
...
function ajaxRestore(id)
{
  ...
  if(confirm( txt['restore-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('delete');
    $('#myForm').attr("action", 'violencelevel/restore/'+id);
    $('#myForm').submit();
  }
  else
  {
...
function ajaxMainRestoreAll()
{
       ...
       else
       {
         if( confirm( txt['restore-confirm'] ))
         {
           ...
           $('#myForm').attr('action','violencelevel/restoreall');
...
function ajaxDelete(id)
{
  ...
  if(confirm( txt['delete-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('delete');
    $('#myForm').attr("action", 'violencelevel/trash/'+id);
    $('#myForm').submit();
  }
  else
  {
...
function ajaxMainDeleteAll()
{
       ...
       else
       {
         if( confirm( txt['delete-confirm'] ))
         {
           ...
           $('#myForm #js_vars').val( id_name );
            $('#myForm').attr('action','violencelevel/trashall');
...
function show_status_modal()
{
  $('#myModal').on('show.bs.modal', function (event) {
    //alert('www');
     ...
     $.get('violencelevel/getstatus/'+s_id,'', function(responseText) {
...
function save_status_modal()
{
  ...
  $('#btnstatus').click(function(event) {
    ...
    $.post('violencelevel/changestatus',js_vars, function(responseText) {
        //console.log(js_vars);
        console.log(responseText)
  ...
  if(countCheckbox() == 1)
  {
    if(confirm( txt['edit-confirm']  ))
    {
        var data_id     =  $('input[name="checkboxID[]"]:checked').val();
         var url           = 'violencelevel/'+data_id+'/edit';
  ...
  else
  {
    if(confirm( txt['softdelete-confirm']  ))
    {
         ...
         var id_name = [];
          ...
          $('#myForm #js_vars').val( id_name );
          $('#myForm').attr('action','violencelevel/softdeleteall');
          $('#myForm').attr('_method','POST');
          $('#myForm').submit();
          return true;
    }
