...
function ajaxEdit(id)
{
  ...
  if(confirm( txt['edit-confirm']  ))
  {
    ...
    $('#myForm').attr("action", 'division/'+id+'/edit');
    $('#myForm').submit();
...
function ajaxShow(id)
{
  ...
  if(confirm( txt['show-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('get');
    $('#myForm').attr("action", 'division/'+id);
    $('#myForm').submit();
...
function ajaxRestore(id)
{
  ...
  if(confirm( txt['restore-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('delete');
    $('#myForm').attr("action", 'division/restore/'+id);
    $('#myForm').submit();
  }
  else
  {
...
function ajaxMainRestoreAll()
{
       ...
       else
       {
         if( confirm( txt['restore-confirm'] ))
         {
           ...
           $('#myForm').attr('action','division/restoreall');
...
function ajaxDelete(id)
{
  ...
  if(confirm( txt['delete-confirm']  ))
  {
    ...
    $('#myForm input[name="_method"]').val('delete');
    $('#myForm').attr("action", 'division/trash/'+id);
    $('#myForm').submit();
  }
  else
  {
...
function ajaxMainDeleteAll()
{
       ...
       else
       {
         if( confirm( txt['delete-confirm'] ))
         {
           ...
           $('#myForm #js_vars').val( id_name );
            $('#myForm').attr('action','division/trashall');
...
function show_status_modal()
{
  $('#myModal').on('show.bs.modal', function (event) {
    //alert('www');
     ...
     $.get('division/getstatus/'+s_id,'', function(responseText) {
...
function save_status_modal()
{
  ...
  $('#btnstatus').click(function(event) {
    ...
    $.post('division/changestatus',js_vars, function(responseText) {
        //console.log(js_vars);
        console.log(responseText)
  ...
  else
  {
    if(confirm( txt['softdelete-confirm']  ))
    {
         ...
         var id_name = [];
          ...
          $('#myForm #js_vars').val( id_name );
          $('#myForm').attr('action','division/softdeleteall');
          $('#myForm').attr('_method','POST');
          $('#myForm').submit();
          return true;
    }
