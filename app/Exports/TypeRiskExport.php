...
class TypeRiskExport implements FromView
{
  protected $typerisk_id;
  protected $division_id;
  protected $subdivision_id;

    public function __construct($typerisk_id,$division_id,$subdivision_id,$urlse_date)
    {
      list($sdate,$edate) =  explode("_" ,$urlse_date);
      $this->typerisk_id = $typerisk_id;
...
$data = Incident::where([['headrm_sendto_headdivision_status','Y'],['headrm_delete','=','']]);

      if($this->typerisk_id !='0')
      {
          $data->where('type_risk_id','=',$this->typerisk_id) ;
      }
      ...
      return view('reports.typerisk.export')->with('data',$data);
