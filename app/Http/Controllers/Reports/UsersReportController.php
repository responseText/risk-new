...
class UsersReportController extends Controller
{
  public function index(Request $request)
  {
      ...
      if(isset($request->filterdaterage))
      {
          ...
          if( $request->filterdaterage != '0' || !empty($request->filterdaterage) )
          {
              ...
              $data =DB::select(
                "select u.id,
                      ...
                      where (i.by_user_id = u.id) and headrm_delete='' and (i.incident_date between '".$param_s."' and  '".$param_e."')
                    ) as Count
                  	from users u
                  Order by Count desc"
                  );
                  return view('reports.usersreport.index')->with('data',$data)->with('url_date' , $url_date );
          ...
          else
          {
              $param_s = substr(Carbon::now(),1,10);
              $param_e = substr(Carbon::now(),1,10);
              return view('reports.usersreport.index')
      ...
      {
          return view('reports.usersreport.index')->with('url_date' , $url_date );
      }
  ...
  public function exportExcel($originaldate)
  {
    ...
    return Excel::download(new UsersReportExport($originaldate), 'report.xlsx' );
