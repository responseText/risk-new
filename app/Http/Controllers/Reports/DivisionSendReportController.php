...
use App\Exports\DivisionSendReportExport;
class DivisionSendReportController extends Controller
{
  ...
  ORDER BY d.id asc
                  ...
                  return view('reports.divisionsendreport.index')->with('data',$data)->with('url_date' , $url_date );
          ...
          else
          {
              $param_s = substr(Carbon::now(),1,10);
              $param_e = substr(Carbon::now(),1,10);
              return view('reports.divisionsendreport.index')
      ...
      {
          return view('reports.divisionsendreport.index')->with('url_date' , $url_date );
      }
  ...
  public function exportExcel($originaldate)
  {
    ...
    return Excel::download(new DivisionSendReportExport($originaldate), 'report.xlsx' );
