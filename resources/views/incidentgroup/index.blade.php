...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">หมวดหมู่อุบัติการณ์ </h3>
  </div>
  <!-- /.box-header -->
  ...
  <div class="box-body">
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
        ...
        {
        ?>
        <a href="{{route('incidentgroup.create')}}" class="btn btn-primary">
...
?>
    <br>
    <br>
    <form id="myFormSearch" class="" action="{{route('incidentgroup.index')}}" method="post">
    ...
    <div class="box box-info ">
        ...
        <div class="box-body">
          ...
          <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
                ...
                <select class="form-control select2" name="selectProgram"><?php if(isset($request->selectprogram)){ echo $request->selectprogram; }?>

                  <option value="">*** โปรแกรมความเสี่ยง ***</option>
                  <?php foreach ($incidentprogram as $rs)
                  {
                  ?>
                  <option value="{{$rs->id}}" <?php if( isset($selectprogramid)){ if( $selectprogramid==$rs->id){ echo 'selected="selected"';} }?>>
                    {{$rs->name}}
                  </option>
                  <?php
                  }
        ...
        <div class="box-footer">
          <a href="{{route('incidentgroup.index')}}" class="btn btn-default">ล้าง</a>
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
        ...
        <table id="example2" class="table table-bordered table-hover table-responsive" >
                ...
                <tr>
                  ...
                  <th class="text-center">โปรแกรมความเสี่ยง</th>
                  <?php
                  if( in_array('1',$arr_user_level) || in_array('2',$arr_user_level) )
                  {
                  ?>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <a href="<?=route('incidentgroup.show',[$rs->id])?>" >
                    <?php echo $rs->name;?>
                    </a>
                  </td>
                  <td>
                  <?php
                  echo $rs->risk_program->name;
                  ?>
                </td>
                <?php
                if( in_array('1',$arr_user_level) || in_array('2',$arr_user_level) )
...
<?php
 if( count($data) > 0 ){
 ?>
 <form id="myStatusForm" name="myStatusForm"  action="{{action('IncidentGroupController@changestatus')}}"  method="POST" >
 <input type="hidden" name="_method" value="POST">
 <input type="hidden" name="_token" value="{{ csrf_token() }}">
       ...
       <div class="modal-body">
         <div class="form-group">
             <div class="radio  has-success">
               <label>
                 <input type="radio" name="status" id="status_enable" value="enable"  class="minimal"   >
                 <i class="fa fa-check" style="color:green;"></i>&nbsp;{{ trans('buttons.enable')}}
               </label>
             </div>
             <div class="radio has-error">
               <label>
                 <input type="radio" name="status" id="status_disable"  value="disable"  class="minimal" >
...
<script type="text/javascript" src="{{asset('js/system/incidentgroup/js-handle1.js')}}"></script>
...
<script type="text/javascript" src="{{asset('js/system/incidentgroup/js-handle.js')}}"></script>
<?php
