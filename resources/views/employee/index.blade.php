...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">ข้อมูลเจ้าหน้าที่ </h3>
  </div>
  <!-- /.box-header -->
...
<?php
 if( count($data) > 0 ){
 ?>
 <form id="myStatusForm" name="myStatusForm"  action="{{action('EmployeeController@changestatus')}}"  method="POST" >
 <input type="hidden" name="_method" value="POST">
 <input type="hidden" name="_token" value="{{ csrf_token() }}">
...
<script type="text/javascript" src="{{asset('js/system/employee/js-handle.js')}}"></script>
<script>
